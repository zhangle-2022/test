...
// var gourl = encodeURI(parent.location.href);
...
$(function (){
    ...
    idm.user.logout({
      success:function(){	// 回调函数（成功）
        if(gourl!=null&&gourl!=""){
          ...
          parent.location.href = 'https://user.www.gov.cn';
        }
      },
      fail:function(r)	{	// 回调函数(失败)
  ...
  $('#godesktop').on('click', function () {
    ...
    // window.top.location.href = url.indexOf('?desk=true') >=0 ? url : url += '?desk=true';
    function setSessionCookie(name, value, cookiePath) {
      if (value) {
        ...
        parent.window.document.cookie = name + "=" + escape(value) + expire + path;
      }
    }
